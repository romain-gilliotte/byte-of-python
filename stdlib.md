...
Nous utilisons la fonction `os.path.join()` pour concaténer ces trois parties de l'emplacement. Nous utilisons une fonction spéciale plutôt qu'un simple ajout de chaînes de caractères, parce que cette fonction va s'assurer que l'emplacement complet correspond au format attendu par le système d'exploitation. Note: la méthode `join()` que nous utilisons ici fait partie du module `os`, ce n'est pas la même que la méthode `join()` disponible sur les chaînes de caractères que nous avons utilisée ailleurs dans ce livre.
